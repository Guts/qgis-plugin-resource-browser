...
def compileResourceFiles(
    ...
    assert dirRoot.is_dir(), f'"dirRoot" is not a directory: {dirRoot}'
        ...
        for i in range(includeNodes.count()):
            ...
            if "location" in attr.keys():
                ...
                if not qrc_path.exists():
                    if (
                        REGEX_QGIS_IMAGES_QRC.match(qrc_path.as_posix())
                        and skip_qgis_images
                    ):
                        continue
                    info = [
                        f"Broken *.qrc location in {ui_file}",
                        f' `location="{location}"`',
                    ]
    ...
    if len(qrc_files) == 0:
        print(f"Did not find any *.qrc files in {dirRoot}", file=sys.stderr)
        return

    print(f"Compile {len(qrc_files)} *.qrc files:")
...
def compileResourceFile(
    ...
    cmd2 = f"pyrcc5 -no-compress -o {pathPy.as_posix()} {pathQrc.name}"
    # print(cmd)
    ...
    if True:
        ...
        assert PyQt5.pyrcc_main.processResourceFile(
            [pathQrc.name], pathPy.as_posix(), False
        )
...
def compileQGISResourceFiles(
    qgis_repo: Union[str, pathlib.Path, None], target: str = None
):
    """
    ...
    assert (
        qgis_repo / "images" / "images.qrc"
    ).is_file(), f"{qgis_repo.as_posix()} is not the QGIS repository root"
